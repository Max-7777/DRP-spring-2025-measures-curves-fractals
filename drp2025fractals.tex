...
\begin{definition}[Interval length]
	Given an interval $I$ of the form $[a,b]$, $(a,b)$, $(a,b]$, or $[a,b)$ where $a < b$, the length of $I$ is 
	\[b - a\]
\end{definition}
This is logical and useful. But how can this be justified with some definition of measure? And how can the idea of length be defined in a more general way? 
\\[0.2in]Most definitions of measure and dimension involve the idea of a $\textit{covering}$.
...
\begin{example}[Open covering for an interval]
	...
	\[C = \{(-50,0),(-20,70),(50,100)\}\]
	since 
	\[I \subseteq \bigcup_{i=1}^{3} C_i\]
	and every set $C_i$ in $C$ is open.
	\\[0.2in]You could say that $C$ is ``overkill'' as an open cover for $I$ since it spans a much longer length than $I$. In this case, the length of $I$ is simply
	\[3 - 1 = 2\]
	and the length of the open cover $\bigcup_{i=1}^{3} C_i$ is
	\[100 - (-50) = 150\]
\end{example}
...
\begin{example}[Stick cutting]
	...
	\\[0.2in]Unfortunately this question doesn't make sense. There is no ``end'' to the cutting process since it goes on forever. First the object should be defined in a valid way. Say $s_0$ is the stick before cutting, $s_1$ is the stick at the first cutting step etc. We can define what we want to measure as:
	...
	\\[0.2in]Another object which represents the length of the stick is
	\[\lim_{i \rightarrow \infty} |s_i|\]
	\\In other words, the length of each stick of the sequence as it goes towards infinity.
...
\newpage
\begin{figure}
	\includegraphics[scale=0.3]{img/c6.jpg}
	\caption{The sequence of construction of some intervals. On each step, each interval is cut and half and the right half is moved towards the right.}
\end{figure}
